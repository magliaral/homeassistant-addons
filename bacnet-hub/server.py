...
# Hinweis: Früher wurde hier pauschal _debug in allen bacpypes3-Modulen aktiviert.
# Für dein gewünschtes Verhalten (nur CLI-`--debug` ohne Modulliste) lassen wir das weg.
...
def _build_argv_from_yaml(config: Dict[str, Any]) -> List[str]:
    ...
    # Testweise: bei Debug-Loglevel automatisch ein nacktes --debug ergänzen
    # (ohne Modulliste), damit das CLI-Verhalten nachgebildet wird.
    try:
